...
from resp_ews.utils.statistics import lr_slope
...
def print_nan_stats(in_arr):
    ''' Print NAN statistics/ value statistics about an array
        REFACTORING: Put to utility module'''
...
def time_window_mean_std_trend(in_arr):
    ...
    trend_vect=np.apply_along_axis(lambda slice_arr: lr_slope(slice_arr),0,in_arr).flatten()
