...
def get_filtered_patient_ids(version='v6b'):
    return np.sort(pd.read_csv(join(datapath, 'misc_derived', 'id_lists', version, 'patients_filtered.csv')).PatientID.values)
...
def read_single_patient_from_merged(patient_id, vname_list=None, verbose=False):
    ...
    if verbose:
        print('Time to read patient_id=%d: %g sec'%(patient_id, (time()-t)))
        t = time()
    df = pd.concat(df, axis=1, join_axes=[df[0].index])
    if verbose:
        print('Time to concate patient_id=%d: %g sec'%(patient_id, (time()-t)))
    return df
