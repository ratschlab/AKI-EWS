...
def LoadRawHDF5Values(tbl_name, 
    ...
    if tbl_name == 'pharmarec':
        df = df[['PatientID', datetime_col, 'VariableID', 'InfusionID', 'Rate', 'Value', 'Status']+(['EnterTime'] if datetime_col=='Datetime' else [])]
    else:
        df = df[['PatientID', datetime_col, 'VariableID', 'Value']+(['EnterTime'] if datetime_col=='Datetime' else [])]
...
def main(tbl_name, index_chunk, output_to_disk):
    ...
    for i in range(num_pid):
        ...
        df_tbl_et = LoadRawHDF5Values(tbl_name, pid_list[i], variable_ids=vid_list, datetime_col='EnterTime', verbose=True)
        t_read = time() - t
        ...
        if len(df_tbl) > 0:
            ...
            df_mat_et = table2matrix(tbl_name, df_tbl_et, variables_of_interest)
            t_pivot = time() - t
            ...
            if tbl_name != 'pharmarec':
                try:
                    ...
                    # assert(len(df_mat_et)==len(df_tbl_et.drop_duplicates(['Datetime'])))
                ...
                try:
                    ...
                    # assert(len(df_tbl_et.VariableID.unique())==np.sum(np.sum(pd.notnull(df_mat_et.iloc[:,2:]), axis=0)>0))
                except AssertionError:
            ...
            sum_tbl_value_et = np.sum(df_tbl_et.Value)
            try:
                assert(sum_tbl_value==sum_tbl_value_et)
            except:
                ipdb.set_trace()
            if tbl_name == 'pharmarec':
                sum_mat_value = 0
                ...
                sum_mat_value_et = 0
                non_zero_drugs = df_mat_et.dropna(axis=1, how='all').columns[2:]
                for col in non_zero_drugs:
                    df_drug = df_mat_et.loc[:, ['Datetime', col]]
                    df_drug.set_index('Datetime', inplace=True)
                    df_drug.sort_index(inplace=True)
                    df_drug.dropna(inplace=True)
                    if df_drug.shape[0] < 2:
                    ...
                    tdiff = ((df_drug.index[1:] - df_drug.index[:-1]).astype('timedelta64[s]').values)/60
                    total_dose = np.dot(df_drug.values[:-1].reshape(-1, ), tdiff)
                    sum_mat_value_et += total_dose
                    
            else:
                sum_mat_value = np.nansum(df_mat.iloc[:,2:])
                sum_mat_value_et = np.nansum(df_mat_et.iloc[:,2:])

            try:
                # NOTE: due to rescaling, this should not be true for many pharma records
                assert(np.abs(sum_tbl_value_et - sum_mat_value_et) < 1e-4)
            except:
                tmp = df_tbl[df_tbl.duplicated(['EnterTime', 'VariableID'], keep=False)].copy()
                tmp = tmp[['PatientID', 'VariableID', 'EnterTime', 'Datetime', 'Value']]
                for dt in tmp.EnterTime.unique():
                    for vid in tmp[tmp.EnterTime==dt].VariableID.unique():
                        print(tmp[(tmp.EnterTime==dt)&(tmp.VariableID==vid)])
                        ipdb.set_trace()
            ...
            df_mat_et.rename(columns={col: col+'_et' for col in df_mat_et.columns if col[0] in ['p', 'v']}, inplace=True)
            df_mat = df_mat.merge(df_mat_et[['Datetime']+[col for col in df_mat_et.columns if col[0] in ['p', 'v']]], how='outer', left_on='Datetime', right_on='Datetime')
            df_mat.loc[:,'PatientID'] = pid_list[i]
